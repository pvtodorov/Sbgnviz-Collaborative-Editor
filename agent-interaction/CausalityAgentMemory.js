...
CausalityAgent.prototype.init = function(){
    ...
    //Read pnnl data from the server
     //   this.sendRequest('agentPNNLRequest', null, function(pnnlList){
      //  self.pnnl = pnnlList;
       // console.log("Pnnl data received");
        //
        // var pnnlArr = [];
        // for(var att in pnnlList){
        //     if(pnnlList.hasOwnProperty(att))
        //         pnnlArr.push(JSON.stringify(pnnlList[att]));
        // }
        //saveFile(pnnlArr, "pnnlSimplified", "text/plain");

        var  pnnlDb = createPNNLDatabase("pnnl");
        pnnlDb.init(pnnlList);
...
CausalityAgent.prototype.performAction = function(gene, callback){
    ...
    if (indCausal > -1){
        agentMsg = "The largest explainable correlation about  " + gene + " is the correlation with " + corrArr[indCorr].id2 + " with a value of " + maxCorr + ". ";
        ...
        for (var j = 0; j < corrArr.length; j++) {
            ...
            if (self.causality[gene].indexOf(geneCorr) < 0) {
                ...
                corrVal = corrArr[j].correlation;
            }
            
        }
        
        if(geneNonCausal){
            agentMsg += "I can't find any causal relationships about " + geneNonCausal + " although it has a correlation of " + corrVal + " with " +  gene + ". ";

            var commonUpstreams = self.findCommonUpstreams(gene, geneNonCausal);
            if (commonUpstreams)
                agentMsg += makeUpstreamStr(commonUpstreams);
            else
                agentMsg += "They don't have any common upstreams.";
        ...
        var corr = corrArr[0].correlation; //corrArr is sorted so take the first element
        if(corr)
            agentMsg += "But it has the highest correlation with " + corrArr[0].id2 + " in PNNL data. ";

        //Search common upstreams of gene and its correlated
        var commonUpstreams = self.findCommonUpstreams(gene, corrArr[0].id2.split('-')[0]);
...
function makeUpstreamStr(commonUpstreams){
    ...
    if(upstreamStr){
        ...
        agentMsg = "They don't have a common upstream. ";
