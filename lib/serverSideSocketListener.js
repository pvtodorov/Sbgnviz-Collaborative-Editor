...
module.exports.start = function(io, model, cancerDataOrganizer){
    ...
    var listenToAgentRequests = function(socket){
        ...
        // socket.on('agentReadPNNLDataRequest', function(callback){
        //     //init data
        //     cancerDataOrganizer.readOvarianPNNLData(model);
        //
        // });
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('subscribeHuman', function (data,  callback) {
            ...
            model.subscribe('pnnl.values', function(){
              ...
              //  model.remove('pnnl.data',0, dataLength);
              //  console.log(model.get('pnnl.values'));

                //DO THIS ONLY ONCE
                //Read this into database
                var READ_DATA = false;
                if(READ_DATA) {
                    ...
                    // console.log(model.get('pnnl.data').length);

                    var readline = require('readline');
                    var stream = require('stream');
                    var fs = require('fs');
                    var filePath = './agent-interaction/CausalPath/PNNL-ovarian-correlations.txt';
                    ...
                    rl.on('line', function (line) {
                        ...
                        if (id1.indexOf('/') < 0 && id2.indexOf('/') < 0) {
                            pnnlArray.push({id1: id1, id2: id2, correlation: vals[2], pVal: vals[3]});
                            console.log(i);
                    ...
                    rl.on('close', function () {
                        ...
                        for(var j = 0; j < pnnlArray.length; j++) {

                            console.log(j );
                      //      model.set('pnnl.values.'+ pnnlArray[j].id1 + '.' + pnnlArray[j].id2 +'.correlation',  pnnlArray[j].correlation);
                         //   model.set('pnnl.values.'+ pnnlArray[j].id1 + '.' + pnnlArray[j].id2 +'.pVal',  pnnlArray[j].pVal);
                          //  console.log(model.get('pnnl.data'));
                        //    model.push('pnnl.data', pnnlArray[j]);

                            // console.log(model.get('pnnl.data').length);
                        }
                        // //console.log(pnnlArray);
            ...
            model.subscribe('documents', function () {
                ...
                //Notify agents of model changes
                    ...
                    model.on('all', (docPath + '.messages.**'), function (id, op, msg) {
                        //update the date
                        // msg.date = (new Date);
                        // model.set(docPath + '.messages.' + id, msg);
                        ...
                        //     if (msg.targets) {
                        //         if(msg.targets === '*' || msg.targets ==='all') {
                        //             var obj = model.get(docPath + '.users');
                        //         }
                        //         else{
