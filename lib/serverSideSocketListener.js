...
module.exports.start = function(io, model, cancerDataOrganizer){
    ...
    var listenToAgentRequests = function(socket){
        ...
        socket.on('agentUndoRequest', function(data, callback){ //from computer agent
            ...
            // modelManagerList[data.room].printHistory();
            //
            //
            // var actionStr = modelManagerList[data.room].getUndoActionStr();
        ...
        socket.on('agentSetLayoutProperties', function(data){
            modelManagerList[data.room].setLayoutProperties(data);
        });
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('subscribeHuman', function (data,  callback) {
            ...
            model.subscribe('documents', function () {
                ...
                model.on('change', 'documents.' + data.room + '.undoIndex', function(id, cmdInd){
                    console.log("undo index changes here as well");

                    var cmd = model.get('documents.' + data.room  + '.history.' + id);
                    modelOp = cmd.opName;
                    console.log(modelOp);
