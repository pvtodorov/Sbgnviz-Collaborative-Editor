...
module.exports.start = function(io, model, cancerDataOrganizer){
    ...
    var readPNNLData = function(data, callback){
        if(Object.keys(pnnlList).length === 0) {
            //READ PNNL Data
            ...
            rlCorr.on('line', function (line) {
                ...
                if (id1.indexOf('/') < 0 && id2.indexOf('/') < 0) { //exclude incorrect formats
                    ...
                    if( pnnlList[id1])
                        pnnlList[id1].push({ id2: id2,correlation: vals[2], pVal: vals[3]});
                    else
                        pnnlList[id1] = [{ id2: id2,correlation: vals[2], pVal: vals[3]}];
