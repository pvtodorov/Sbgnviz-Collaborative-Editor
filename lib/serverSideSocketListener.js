...
module.exports.start = function(io, model, cancerDataOrganizer){
    ...
    var listenToAgentRequests = function(socket){
        ...
        socket.on('agentPNLLRequest', function(data, callback){
            // var READ_DATA = true;
            // if(READ_DATA) {

            console.log("Received request");

            if(pnnlArray.length <=0 ) {
                //READ PNNL Data
                ...
                rlCorr.on('close', function () {
                    ...
                    if (callback) callback(pnnlArray);
                ...
                // //Read causative.sif
                // var filePathSif = './agent-interaction/CausalPath/causative.sif';
                // var instreamSif = fs.createReadStream(filePathSif);
                // var rlSif = readline.createInterface(instreamSif);
                //
                //
                // var i = 0;
                // var causativeArr = [];
                // rlSif.on('line', function (line) {
                //
                //     var vals = line.split("\t");
                //     causativeArr.push({id1: vals[0], rel:vals[1], id2:vals[2], pcUri:vals[3]});
                //     i++;
                // });
                //
                //
                // rlSif.on('close', function () {
                //     console.log("PNNL data reading into memory complete.");
                //
                // });
