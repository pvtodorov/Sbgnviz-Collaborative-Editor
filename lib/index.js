...
//                modelManagerList[data.room].addModelNode(data.id, data.param);
            ...
            socket.on('agentAddEdgeRequest',function(data,  callback){
               ...
               var status = modelManagerList[data.room].addModelEdge(data.id, data.param);
            ...
            socket.on('agentDeleteNodeRequest',function(data, callback){
                var status = modelManagerList[data.room].deleteModelNode(data.id);
                if(callback) callback(status);

            });
            socket.on('agentDeleteEdgeRequest',function(data, callback){
                var status = modelManagerList[data.room].deleteModelEdge(data.id);
                if(callback) callback(status);
            });

            socket.on('agentMoveNodeRequest',function(data, callback){

                var status = modelManagerList[data.room].moveModelNode(data.id, data.pos);
                if(callback) callback(status);
            ...
            socket.on('agentHighlightRequest', function(data, callback){
                ...
                var status = modelManagerList[data.room].highlight(data.val);
                if(callback) callback(status);
            });
