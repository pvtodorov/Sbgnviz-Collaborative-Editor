...
function run(app, options, cb) {
  ...
  function listenCallback(err) {
    ...
    function createServer() {
        ...
        var useBiopax = true;
        var pcServletAddress =  "http://localhost:8080/PCServlet";
        ...
        io.sockets.on('connection', function (socket) {
            ...
            socket.on('agentLoadFileRequest',  function(data, callback){
                ...
                if(data.fileType == "owl"){
                    ...
                    request.post({
                        url: "http://localhost:8080/PaxtoolsServer",
            ...
            socket.on('agentAddNodeRequest',function(data, callback){
                ...
                if(useBiopax) {
                    request.post({
                        url: pcServletAddress,
                        headers: responseHeaders,
                        form: {reqType: "addNode", content: JSON.stringify(data.param)}
                    }, function (error, response, body) {
                        ...
                        if (error) {
                            ...
                            if(body.indexOf("OK")>-1 && response.statusCode == 200){

                                console.log(body);
                                var nodeId = menuList[data.userId].addNode(data.param.id, data.param.x, data.param.y, data.param.sbgnclass, data.param.sbgnlabel, true);

                                if (callback) {

                                    if(!nodeId)
                                        callback("Cannot add a second node with the same id.");
                                    else
                                        callback(nodeId);
                                }
                                ...
                                if (callback)
                                    callback("Node adding error: " + response.statusCode + " " + body);
                            }
            ...
            socket.on('agentChangeNodeAttributeRequest', function(data, callback){
                ...
                if(useBiopax) {
                    ...
                    if (data.attStr == "unitOfInformation" || data.attStr == "stateVariables") { //make changes for every state variable


                        var formVal;

                        if (data.attStr== "unitOfInformation") {
                             formVal = {reqType:data.attStr, id: data.id, unitOfInformation: data.unitOfInformation};
                        }
                        ...
                        request.post({
                            ...
                            if (error) {
                                if (callback)
                                    callback("Node attribute changing error: ");
                                ...
                                if (body.indexOf("OK") > -1 && response.statusCode == 200) {
                                    ...
                                    var status = modelManagerList[data.userId].changeModelNodeAttribute("sbgnStatesAndInfos", data.id, data.attVal);
                                    if (callback) callback(status);
                                }
                                else {
                                    if (callback)
                                        callback("Node attribute changing error: " + response.statusCode + " " + body);
                                }
                            }
                        });
                    }
                    ...
                    if(data.attStr == "unitOfInformation" || data.attStr == "stateVariables" )
                        var status = modelManagerList[data.userId].changeModelNodeAttribute("sbgnStatesAndInfos", data.id, data.attVal);
                    else
                        var status = modelManagerList[data.userId].changeModelNodeAttribute(data.attStr, data.id, data.attVal);
                    if (callback) callback(status);
            ...
            socket.on('subscribeAgent', function (data, callback) {
             ...
             //   model.on('all', (docPath+ '.cy.nodes.*.addedLater'), function(id, op, idName, prev, passed) { //this property must be something that is only changed during insertion
            ...
            socket.on('PCQuery', function(queryData){
                ...
                request(queryData , function (error, response, body) {
                    ...
                    if (error) {
                        ...
                        console.log(response.statusCode);
                        if(response.statusCode == 200) {
                            socket.emit("PCQueryResult", {graph:body});
                        }
                        else
            ...
            //     socket.on('addNode', function(param, callback){
            //     var request = require('request'); //REST call over http/https
            //     request.post({
            //         url: "http://localhost:8080/SBGNConverterServlet",
            //         headers: responseHeaders,
            //         form: {reqType: "addNode", content: param}
            //     }, function(error, response, body){
            //
            //
            //
