    ...
    function createServer() {
      ...
      require('./server').setup(app, options, function (err, expressApp, upgrade, refModel) {
        ...
        io.sockets.on('connection', function (socket) {
            ...
            socket.on('PCQuery', function(queryParams){
                ...
                var responseContent = "";
                request({
                    url: 'http://www.pathwaycommons.org/pc2/graph', //URL to hit
                    // qs: {from: 'blog example', time: +new Date()}, //Query string data
                   // qs : "q=Q06609&output=sbgnml",//("q=" + queryParams.name + "&format=sbgnml"),
                    method: 'POST',
                    headers: responseHeaders,
                    form:queryParams
                    //Lets post the following key/values as form
