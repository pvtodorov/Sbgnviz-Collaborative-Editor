    ...
    function createServer() {
      ...
      require('./server').setup(app, options, function (err, expressApp, upgrade, refModel) {
        ...
        io.sockets.on('connection', function (socket) {
            ...
            socket.on('agentRemoveNodeRequest',function(data){
                var nodePath = 'documents.' + data.room + '.cy.nodes.' + data.id;
                model.del(nodePath);
            ...
            socket.on('agentOperationHistoryRequest', function(room){ //from computer agent
            ...
            socket.on('subscribeAgent', function (data) {
                ...
                userList.push(data);
                io.in(socket.room).emit('userList',  userList.filter(function (obj) { //tell this to human agents
                    return(obj.room == socket.room);
                }));
