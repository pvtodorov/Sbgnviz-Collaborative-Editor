...
module.exports = function(){
    ...
    return{
            ...
            request("http://www.cbioportal.org/webservice.do?cmd=getCancerStudies", function (error, response, body) {
                ...
                if (error) {
                    ...
                    lines.forEach(function (line) {
                        ...
                        if(words[0].length > 0) {
                            self.cancerStudies.push({id: words[0], name: words[1], seqCaseCnt: 0, mutationCaseIds:[]});

                            self.getSequencedCaseCounts(self.cancerStudies[self.cancerStudies.length-1]);
           ...
           // if (mutationIndex >= 10) { //funda simdilik
            ...
            var req = request(url, function (error, response, body) {
                ...
                if (error) {
                    ...
                    console.log("cBioPortal mutation # " + mutationIndex + ' of ' + cancerStudies.length + " for " + geneList);
        ...
        getAllMutationCounts: function(geneList, callback){
            ...
            var mutationIndex = 0;
            //initialize
            self.cancerStudies.forEach(function(study){
               study.mutationCaseIds.length  = 0;
            });


            //recursive function that increases mutationIndex number each time
            this.getNewMutationData(geneList, self.cancerStudies, mutationIndex, callback);
